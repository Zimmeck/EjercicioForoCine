...
GROUP BY
    p.id_pelicula,
    p.titulo -- Agrupamos para contar por película
ORDER BY Total_Comentarios DESC, -- Ordena de más comentado a menos
    p.titulo;

-- 4. Obtener la media de valoración para cada película
SELECT p.titulo, COALESCE(AVG(v.puntuacion), 0.0) AS Valoracion_Media
FROM PELICULA p
    LEFT JOIN VALORACION v ON p.id_pelicula = v.id_pelicula
GROUP BY
    p.id_pelicula,
    p.titulo -- Agrupamos por el ID y el título de la película
ORDER BY Valoracion_Media DESC, p.titulo;
...
FROM PLATAFORMA pl
    LEFT JOIN (
        SELECT d.id_pelicula, d.id_plataforma
...
FROM
    PELICULA p
    JOIN DISPONIBILIDAD d ON p.id_pelicula = d.id_pelicula
    JOIN PRECIO pr ON d.id_disponibilidad = pr.id_disponibilidad
WHERE
    pr.valor > 5
ORDER BY p.titulo;
...
GROUP BY
    p.id_pelicula,
    p.titulo
ORDER BY Valoracion_Media_Maxima DESC
LIMIT 1;
...
FROM
    COMENTARIO c
    INNER JOIN VALORACION v ON c.id_valoracion = v.id_valoracion
    INNER JOIN PELICULA p ON c.id_pelicula = p.id_pelicula
    INNER JOIN USUARIO u ON c.email_usuario = u.email
WHERE
    v.puntuacion = 10
    AND c.id_valoracion IS NOT NULL
ORDER BY p.titulo, c.fecha;
...
FROM PELICULA_DIRECTOR
WHERE
    id_pelicula = 100
UNION ALL
SELECT 'PELICULA_GUIONISTA', COUNT(*)
FROM PELICULA_GUIONISTA
WHERE
    id_pelicula = 100
UNION ALL
...
DELETE FROM PELICULA WHERE id_pelicula = 100;